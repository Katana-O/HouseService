...
func (e *House) PubHouse(ctx context.Context, req *house.Request, rsp *house.Response) error {
	fmt.Println("PubHouse 1")
	//上传房屋业务  把获取到的房屋数据插入数据库
	houseId, err := model.AddHouse(req)
	if err != nil {
		fmt.Println("PubHouse guess enter 1")
		rsp.Errno = utils.RECODE_DBERR
		rsp.Errmsg = utils.RecodeText(utils.RECODE_DBERR)
		fmt.Println("PubHouse guess enter 2")
		return nil
	}
	...
	rsp.Data = &h
	fmt.Println("PubHouse 2")
	return nil
}

func (e *House) UploadHouseImg(ctx context.Context, req *house.ImgReq, resp *house.ImgResp) error {
	fmt.Println("UploadHouseImg enter")
	//把图片存储到fastdfs中
	//初始化fdfs的客户端
	//fClient, _ := fdfs_client.NewFdfsClient("/etc/fdfs/client.conf")
	wd, _ := os.Getwd()
	fmt.Println("wd:", wd)
	fdfsClient, err := fdfs_client.NewClientWithConfig("../user/conf/fdfs.conf")
	if err != nil {
		fmt.Println("UploadHouseImg 1 err:", err)
		return err
	}

	//上传图片到fdfs
	remoteID, err := fdfsClient.UploadByBuffer(req.ImgData, req.FileExt[1:])
	if err != nil {
		fmt.Println("UploadHouseImg 2 err:", err)
	...
	img.Url = "http://192.168.1.161:8888/" + remoteID
	resp.Data = &img
...
func (e *House) GetHouseInfo(ctx context.Context, req *house.GetReq, resp *house.GetResp) error {
	//根据用户名获取所有的房屋数据
	houseInfos, err := model.GetUserHouse(req.UserName) // problem occurs.
	if err != nil {
		resp.Errno = utils.RECODE_DBERR
		resp.Errmsg = utils.RecodeText(utils.RECODE_DBERR)
		return nil
